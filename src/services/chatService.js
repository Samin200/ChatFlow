...
export function getChatThemePreference(userId, contactId) {
  ...
  // Try unified global keys first if per-chat prefs are empty/default
  const globalBgType = getGenericThemeValue(getChatThemeKeys().bgType);
  const globalBgValue = getGenericThemeValue(getChatThemeKeys().bgValue);
  const globalMine = getGenericThemeValue(getChatThemeKeys().mineBubble);
  const globalTheir = getGenericThemeValue(getChatThemeKeys().theirBubble);

  const themeId = prefs?.themeId ?? "midnight";
  const theme = CHAT_THEMES[themeId] ?? CHAT_THEMES.midnight;

  const backgroundMode = prefs?.backgroundMode ?? globalBgType ?? "solid";
  let solidColor = prefs?.solidColor ?? theme.chatBackground;
  let backgroundStyle = prefs?.backgroundStyle ?? { backgroundColor: solidColor };
  
  if (backgroundMode === "solid") {
    solidColor = prefs?.solidColor ?? globalBgValue ?? theme.chatBackground;
    backgroundStyle = { backgroundColor: solidColor };
  } else if (backgroundMode === "gradient") {
    const from = prefs?.gradientFrom ?? theme.chatBackground;
    const to = prefs?.gradientTo ?? theme.mineBubble;
    const dir = prefs?.gradientDirection ?? "to bottom";
    backgroundStyle = { backgroundImage: `linear-gradient(${dir}, ${from}, ${to})` };
  } else if (backgroundMode === "image") {
    const img = prefs?.backgroundImage ?? globalBgValue;
    backgroundStyle = img ? { backgroundImage: `url(${img})`, backgroundSize: "cover", backgroundPosition: "center" } : { backgroundColor: theme.chatBackground };
  }
  ...
  return {
    ...
    backgroundMode,
    solidColor,
    gradientFrom: prefs?.gradientFrom ?? null,
    gradientTo: prefs?.gradientTo ?? null,
    gradientDirection: prefs?.gradientDirection ?? "to bottom",
    backgroundStyle,
    useCustomSideStyle: Boolean(prefs?.useCustomSideStyle),
    customSideLayout: prefs?.customSideLayout ?? "default",
    mineBubble: prefs?.mineBubble ?? globalMine ?? theme.mineBubble,
    theirBubble: prefs?.theirBubble ?? globalTheir ?? theme.theirBubble,
    showMyAvatarInChat: Boolean(prefs?.showMyAvatarInChat),
    backgroundImage: prefs?.backgroundImage ?? (backgroundMode === "image" ? globalBgValue : null),
  };
}
...
export function setChatAppearance(userId, contactId, patch) {
  ...
  const current = settings.themePrefs[contactId] ?? {};
  const next = { ...current };

  // Handle nulls as "reset to default"
  ...
  // Re-calculate backgroundStyle if needed
  if (patch.backgroundMode || patch.solidColor || patch.gradientFrom || patch.gradientTo || patch.gradientDirection || patch.backgroundImage || patch.backgroundMode === null) {
    const theme = CHAT_THEMES[next.themeId ?? "midnight"];
    const mode = next.backgroundMode ?? "solid";
    
    if (mode === "solid") {
      next.backgroundStyle = { backgroundColor: next.solidColor ?? theme.chatBackground };
    } else if (mode === "gradient") {
      const from = next.gradientFrom ?? theme.chatBackground;
      const to = next.gradientTo ?? theme.mineBubble;
      const dir = next.gradientDirection ?? "to bottom";
      next.backgroundStyle = { backgroundImage: `linear-gradient(${dir}, ${from}, ${to})` };
    } else if (mode === "image") {
      const img = next.backgroundImage;
      next.backgroundStyle = img ? { backgroundImage: `url(${img})`, backgroundSize: "cover", backgroundPosition: "center" } : { backgroundColor: theme.chatBackground };
    }
  }

  settings.themePrefs[contactId] = next;
